;;;; src/image.lisp — data-URI + base64 + PNG/GIF/WebP/SVG decode to RGBA pixels.
;;;;
...
;;;; JPEG (baseline + progressive) lives in jpeg.lisp.
(in-package #:pigment)
...
;;; ---- percent-decode (data: URIs; self-contained) -----------------------
(defun percent-decode (s)
  "Decode %XX escapes in a URI component to a string (each byte as Latin-1;
leaves '+' literal — data: URIs are not form-encoded)."
...
(defun rgba-canvas->img (cv)
  "Convert a premultiplied RGBA scribe canvas to a straight-alpha IMG."
...
(defun decode-image (uri)
  "Decode a data: URI (PNG, GIF, JPEG or SVG) to an IMG, or NIL."
